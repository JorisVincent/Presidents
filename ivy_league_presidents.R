...
uni_data <- uni_data |>
  ...
  mutate(education = ifelse(education %in% c("Harvard University", "Yale University", "Princeton Univeristy", "University of Pennsylvania", "Columbia University"),"Ivy League", education)) |>
...
uni_data |>
  ...
  geom_point(shape = 124, size=15, alpha=0.5, colour='purple') + # shape=124 sets the points to be the pipe-symbol "|"
  theme_light() +
  ylab("") +
  xlab("Year")
ggsave("Barcode_Presidents.pdf", width = 7, height = 4)
